...
vicroad_df = pd.DataFrame(
    {
        'id': vehicle['accident_id'].astype(str) + '-' + vehicle['vehicle_id'].astype(str),
...
atmospheric_dummies = (
    pd.get_dummies(atmospheric_cond, columns=['atmosph_cond'], prefix=['atmosph_cond'])
    .groupby('accident_id').max().reset_index()
)

sub_dca_dummies = (
    pd.get_dummies(sub_dca, columns=['sub_dca_code'], prefix=['sub_dca_code'])
    .groupby('accident_id').max().reset_index()
)

road_surf_cond_dummies = (
    pd.get_dummies(road_surface_cond, columns=['surface_cond'], prefix=['surface_cond'])
    .groupby('accident_id').max().reset_index()
)
...
vicroad_df = (
    vicroad_df
    .merge(accident, on='accident_id', how='left')
    .merge(vehicle, on=['accident_id','vehicle_id'], how='left')
    ...
    .merge(accident_location.drop('node', axis=1, errors='ignore'), on='accident_id', how='left')
)

del [event_dummies, atmospheric_dummies, sub_dca_dummies, road_surf_cond_dummies, person_driver, node_unique,
     accident, vehicle, accident_event, atmospheric_cond, sub_dca, person, node, road_surface_cond, accident_location]
